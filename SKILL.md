...
- 仓库事实：`README`、`TODO.md`、`docs/product/*`、`docs/work-items/**`、`docs/white-tower/status.md`。
...
交付项完成后不能只移动到 `done/`，还必须反写相关全局文档。放弃或过期时移动到 `archived/`，写清原因。
...
- `docs/work-items/<planned|active|done|archived>/**`
- 可用检查、测试、构建命令
...
- 完整扫描后确认没有 pending review、没有可生成/可细化任务、没有可推进交付项、没有缺失技术方案/任务拆解/验收/发布交接、没有 runnable task、没有可修复检查项，并把依据写入 run record 或状态文件。
...
- 用户未确认前保持 `pending_review`，不得把依赖该 UI/UX 的交付项推到源码 dispatch。
- 中断后下次 `继续` / `实施计划` / `dispatch` 必须先读取 `docs/white-tower/status.md` 和交付项内 review 状态；只要有 `pending_review`，就重新发送图片并等待确认。

Review 状态示例：

```yaml
pending_review:
  - id: <work-item-or-page-id>
    image: <path-to-png>
...
每次执行前后至少记录：交付项/task ID、phase、branch、head commit、dirty files、最近动作、下一步、verification 是否在最新 diff 后运行、blocker、risk、manual question。
...
2. 没有白塔状态时，创建最小任务池：
   ...
   - `TODO.md`
   - `docs/work-items/README.md`
   - `docs/work-items/{planned,active,done,archived}/.gitkeep`
...
- 没有 `TODO.md`、交付项包、run record 或 checkpoint，导致下一次只能猜。
- 实现后不更新文档，造成 docs 漂移。
- 很多无关改动挤成一个大提交。
- 把聊天记录当唯一产品真相。
